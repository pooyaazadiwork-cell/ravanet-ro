/**
 * Ro RAG v1.5.1 — ops precision + speed
 */
...
function loadChunks() {
  ...
  const all = [];
  for (const k of ["policy", "safety", "clinical", "skills", "product"]) {
...
function detectIntent(query) {
  ...
  return {
    ...
    skillAsk: /(چیکار|چه\s*کار|راهکار|آروم|نفس|کمک\s*کن)/.test(q),
...
export function formatContext(chunks, maxChars = 1100) {
  ...
  const header = "دانش عملیاتی رو (فقط راهنما؛ تشخیص قطعی نده؛ روش آسیب نده؛ اگر مطمئن نیستی بگو نمی‌دانی):\n\n";
  const parts = [];
...
export function ragStats() {
  return {
    version: "1.5.1",
    chunks: INDEX.length,
    kinds: [...new Set(INDEX.map((c) => c.kind))],
    index: "pretokenized+prefix",
    minScoreDefault: 3,
  };
}
...
export function retrieveDebug(query, k = 4) {
  ...
  return {
    query,
    hits: hits.map((h) => ({ id: h.id, kind: h.kind, title: h.title })),
    contextChars: formatContext(hits).length,
  };
}
